...
export default async function handler(request) {
  ...
  // Your backend URL
  const BACKEND_URL = process.env.BACKEND_URL || 'https://urlshortner-1-hpyu.onrender.com';
  
  console.log(`🌐 Edge Proxy: ${hostname}${pathname} → ${BACKEND_URL}${pathname}`);
  
  // Handle health check
  if (pathname === '/api/proxy/health' || url.searchParams.get('health')) {
  ...
  if (pathname === '/api/proxy/debug' || url.searchParams.get('debug')) {
    return new Response(JSON.stringify({
      timestamp: new Date().toISOString(),
      hostname,
      pathname,
  ...
  // Skip if it's the main frontend domain (avoid infinite loops)
  if (hostname.includes('pebly.vercel.app')) {
    return new Response('This is the main frontend domain', {
      status: 200,
